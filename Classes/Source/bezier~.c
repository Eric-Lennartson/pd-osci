...
{
    t_bezier_tilde *x  = (t_bezier_tilde *)(w[1]);
    t_float *driver_in = (t_float *)(w[2]);
    t_float *x1_in     = (t_float *)(w[3]);
    t_float *y1_in     = (t_float *)(w[4]);
    t_float *x2_in     = (t_float *)(w[5]);
    t_float *y2_in     = (t_float *)(w[6]);
    t_float *x3_in     = (t_float *)(w[7]);
    t_float *y3_in     = (t_float *)(w[8]);
    t_float *x_out     = (t_float *)(w[9]);
    t_float *y_out     = (t_float *)(w[10]);
    int nblock         = (int)(w[11]);
    ...
    while (nblock--)
    {
        if(!x->bypass)
        {
            ...
            x_out[nblock] = x1_in[nblock];
            y_out[nblock] = y1_in[nblock];
        }
    }
    return (w + 12);
}
...
{
    dsp_add(bezier_tilde_perform, 11, x,
            sp[0]->s_vec, // driver
            sp[1]->s_vec, // x1
            sp[2]->s_vec, // y1
            sp[3]->s_vec, // x2
...
static void bypass(t_bezier_tilde *x, t_floatarg f)
{
    f = floorf(f);
    x->bypass = f != 0;
}
...
static void *bezier_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    x->bypass = argc > 6 ? atom_getfloat(argv+5) : false;
...
void bezier_tilde_setup(void)
{
    ...
    class_addmethod(bezier_tilde_class, (t_method)bypass, gensym("bypass"), A_DEFFLOAT, 0);
