...
static t_class *rotate_p_tilde_class;

typedef struct _rotate_p_tilde
{
  t_object x_obj;
...
t_int *rotate_p_tilde_perform(t_int *w)
{
  t_rotate_p_tilde *x = (t_rotate_p_tilde *)(w[1]); // access data space
...
void rotate_p_tilde_dsp(t_rotate_p_tilde *x, t_signal **sp)
{
  dsp_add(rotate_p_tilde_perform, 15, x,
          sp[0]->s_vec, // x_in
          sp[1]->s_vec, // y_in
          sp[2]->s_vec, // z_in
          sp[3]->s_vec, // angle
...
void angle_mode(t_rotate_p_tilde *x, t_symbol* mode)
{
...
static void bypass(t_rotate_p_tilde *x, t_floatarg bypass) {
  x->bypass = (int)bypass;
}

void rotate_p_tilde_free(t_rotate_p_tilde *x)
{
  // inlet_free(x->x_in);
  inlet_free(x->y_in);
  inlet_free(x->z_in);
...
void *rotate_p_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
  t_rotate_p_tilde *x = (t_rotate_p_tilde *)pd_new(rotate_p_tilde_class);
...
void rotate_p_tilde_setup(void)
{
  rotate_p_tilde_class = class_new(gensym("rotate_p~"),
                                       (t_newmethod)rotate_p_tilde_new,
                                       (t_method)rotate_p_tilde_free,
                                       sizeof(t_rotate_p_tilde),
                                       CLASS_DEFAULT,
                                       A_GIMME, // angle, pivot, axis
                                       0);

  class_sethelpsymbol(rotate_p_tilde_class, gensym("rotate~")); // links to the help patch

  class_addmethod(rotate_p_tilde_class, (t_method)angle_mode, gensym("mode"), A_DEFSYMBOL, 0);
  class_addmethod(rotate_p_tilde_class, (t_method)bypass, gensym("bypass"), A_DEFFLOAT, 0);

  class_addmethod(rotate_p_tilde_class, (t_method)rotate_p_tilde_dsp, gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(rotate_p_tilde_class, t_rotate_p_tilde, f); // dummy arg for singal into first inlet
}
