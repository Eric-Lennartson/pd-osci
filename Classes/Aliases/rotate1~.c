...
static t_class *rotate1_tilde_class;

typedef struct _rotate1_tilde
{
  t_object x_obj;
...
t_int *rotate1_tilde_perform(t_int *w)
{
  t_rotate1_tilde *x = (t_rotate1_tilde *)(w[1]); // access data space
...
void rotate1_tilde_dsp(t_rotate1_tilde *x, t_signal **sp)
{
  dsp_add(rotate1_tilde_perform, 11, x,
          sp[0]->s_vec, // x_in
          sp[1]->s_vec, // y_in
          sp[2]->s_vec, // z_in
          sp[3]->s_vec, // xRot_in
...
void angle_mode(t_rotate1_tilde *x, t_symbol* mode)
{
...
void rotate1_tilde_free(t_rotate1_tilde *x)
{
  //    inlet_free(x->x_in);
  inlet_free(x->y_in);
  inlet_free(x->z_in);
...
void *rotate1_tilde_new(t_floatarg ax, t_floatarg ay, t_floatarg az)
{
  t_rotate1_tilde *x = (t_rotate1_tilde *)pd_new(rotate1_tilde_class);
...
void rotate1_tilde_setup(void)
{
  ...
  class_addmethod(rotate1_tilde_class, (t_method)angle_mode, gensym("mode"), A_DEFSYMBOL, 0);
  class_addmethod(rotate1_tilde_class, (t_method)rotate1_tilde_dsp, gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(rotate1_tilde_class, t_rotate1_tilde, f); // dummy arg for singal into first inlet
}
