...
static t_class *rotate_a_tilde_class;

typedef struct _rotate_a_tilde
{
  t_object x_obj;
...
t_int *rotate_a_tilde_perform(t_int *w)
{
  t_rotate_a_tilde *x = (t_rotate_a_tilde *)(w[1]); // access data space
...
void rotate_a_tilde_dsp(t_rotate_a_tilde *x, t_signal **sp)
{
  dsp_add(rotate_a_tilde_perform, 12, x,
          sp[0]->s_vec, // x_in
          sp[1]->s_vec, // y_in
          sp[2]->s_vec, // z_in
          sp[3]->s_vec, // angle_in
...
void angle_mode(t_rotate_a_tilde *x, t_symbol *mode)
{
...
static void bypass(t_rotate_a_tilde *x, t_floatarg bypass) {
  x->bypass = (int)bypass;
}

void rotate_a_tilde_free(t_rotate_a_tilde *x)
{
  //    inlet_free(x->x_in);
  inlet_free(x->y_in);
  inlet_free(x->z_in);
...
void *rotate_a_tilde_new(t_floatarg angle, t_floatarg ax, t_floatarg ay, t_floatarg az)
{
  t_rotate_a_tilde *x = (t_rotate_a_tilde *)pd_new(rotate_a_tilde_class);
...
void rotate_a_tilde_setup(void)
{
  rotate_a_tilde_class = class_new(gensym("rotate_a~"),
                                      (t_newmethod)rotate_a_tilde_new,
                                      (t_method)rotate_a_tilde_free,
                                      sizeof(t_rotate_a_tilde),
                                      CLASS_DEFAULT,
                                      A_DEFFLOAT, // angle
                                      A_DEFFLOAT, // axis X
                                      A_DEFFLOAT, // axis Y
  ...
  class_sethelpsymbol(rotate_a_tilde_class, gensym("rotate~")); // links to the help patch

  class_addmethod(rotate_a_tilde_class, (t_method)angle_mode, gensym("mode"), A_DEFSYMBOL, 0);
  class_addmethod(rotate_a_tilde_class, (t_method)bypass, gensym("bypass"), A_DEFFLOAT, 0);

  class_addmethod(rotate_a_tilde_class, (t_method)rotate_a_tilde_dsp, gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(rotate_a_tilde_class, t_rotate_a_tilde, f); // dummy arg for singal into first inlet
}
