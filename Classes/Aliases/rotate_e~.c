...
static t_class *rotate_e_tilde_class;

typedef struct _rotate_e_tilde
{
  t_object x_obj;
...
t_int *rotate_e_tilde_perform(t_int *w)
{
  t_rotate_e_tilde *x = (t_rotate_e_tilde *)(w[1]); // access data space
...
void rotate_e_tilde_dsp(t_rotate_e_tilde *x, t_signal **sp)
{
  dsp_add(rotate_e_tilde_perform, 11, x,
          sp[0]->s_vec, // x_in
          sp[1]->s_vec, // y_in
          sp[2]->s_vec, // z_in
          sp[3]->s_vec, // xRot_in
...
void angle_mode(t_rotate_e_tilde *x, t_symbol* mode)
{
...
static void bypass(t_rotate_e_tilde *x, t_floatarg bypass) {
  x->bypass = (int)bypass;
}

void rotate_e_tilde_free(t_rotate_e_tilde *x)
{
  //    inlet_free(x->x_in);
  inlet_free(x->y_in);
  inlet_free(x->z_in);
...
void *rotate_e_tilde_new(t_floatarg ax, t_floatarg ay, t_floatarg az)
{
  t_rotate_e_tilde *x = (t_rotate_e_tilde *)pd_new(rotate_e_tilde_class);
...
void rotate_e_tilde_setup(void)
{
  rotate_e_tilde_class = class_new(gensym("rotate_e~"),
                                       (t_newmethod)rotate_e_tilde_new,
                                       (t_method)rotate_e_tilde_free,
                                       sizeof(t_rotate_e_tilde),
                                       CLASS_DEFAULT,
                                       A_DEFFLOAT, //xRot
                                       A_DEFFLOAT, //yRot
                                       A_DEFFLOAT, //zRot
  ...
  class_sethelpsymbol(rotate_e_tilde_class, gensym("rotate~")); // links to the help patch

  class_addmethod(rotate_e_tilde_class, (t_method)angle_mode, gensym("mode"), A_DEFSYMBOL, 0);
  class_addmethod(rotate_e_tilde_class, (t_method)bypass, gensym("bypass"), A_DEFFLOAT, 0);

  class_addmethod(rotate_e_tilde_class, (t_method)rotate_e_tilde_dsp, gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(rotate_e_tilde_class, t_rotate_e_tilde, f); // dummy arg for singal into first inlet
}
