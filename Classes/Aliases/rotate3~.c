...
t_int *rotate3_tilde_perform(t_int *w)
{
  t_rotate3_tilde *x = (t_rotate3_tilde *)(w[1]); // access data space
...
void rotate3_tilde_dsp(t_rotate3_tilde *x, t_signal **sp)
{
  dsp_add(rotate3_tilde_perform, 15, x,
          sp[0]->s_vec, // x_in
          sp[1]->s_vec, // y_in
          sp[2]->s_vec, // z_in
          sp[3]->s_vec, // angle
...
void angle_mode(t_rotate3_tilde *x, t_symbol* mode)
{
...
void rotate3_tilde_free(t_rotate3_tilde *x)
{
  // inlet_free(x->x_in);
  inlet_free(x->y_in);
  inlet_free(x->z_in);
...
void *rotate3_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
  t_rotate3_tilde *x = (t_rotate3_tilde *)pd_new(rotate3_tilde_class);
...
void rotate3_tilde_setup(void)
{
  ...
  class_sethelpsymbol(rotate3_tilde_class, gensym("rotate~")); // links to the help patch

  class_addmethod(rotate3_tilde_class, (t_method)angle_mode, gensym("mode"), A_DEFSYMBOL, 0);
  class_addmethod(rotate3_tilde_class, (t_method)rotate3_tilde_dsp, gensym("dsp"), A_CANT, 0);
  CLASS_MAINSIGNALIN(rotate3_tilde_class, t_rotate3_tilde, f); // dummy arg for singal into first inlet
}
